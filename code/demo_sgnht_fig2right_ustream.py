...
def run_sghmc_with_problem( problem, T, h, A, Bhat, current_q, current_p = None, current_xi = None, u_stream=None ):
  ...
  for t in range(T):
    ...
    if u_stream is None:
      grad_U = dU( q, np.random.randint(problem.N) )
    else:
      grad_U = dU( q, u_stream[t] )
...
def run_sgnht_with_problem( problem, T, h, A, current_q, current_p = None, current_xi = None, u_stream=None ):
  # extract problem functions
  U  = problem.U
  K  = problem.K
  dU = problem.dU
...
if __name__ == "__main__":
  ...
  u_stream = np.zeros( T, dtype=int )
  tau = 1
  c   = 0
  np.random.seed(4)
  for t in xrange(T):
    if c+1 == tau:
      u_stream[t] = np.random.randint(N)
      c = 0
    else:
  ...
  #assert False
  np.random.seed(73) # seed set to find a posterior for gamma similar to paper SGNHT
  problem = generate_1d_gaussian_var_unknown(N=N,Ntilde = Ntilde)
  
  X=problem.X
  iX = np.argsort(X)
  iii = np.hstack( (iX,np.argsort(-X)) )
  idx = u_stream[0]
  for t in xrange(T):
    idx = np.mod( idx + S + np.random.randint(K), 2*N )
    #u_stream[t] = iii[ idx ]
  ...
  #Qsghmc,Psghmc, XIsghmc = run_sghmc_with_problem( problem, T, h, C, Bhat, current_q, current_p = current_p, current_xi = xi, u_stream=u_stream  )
  Qsgnht,Psgnht, XIsgnht = run_sgnht_with_problem( problem, T, h, A, current_q, current_p = current_p, current_xi = xi,u_stream=u_stream  )
  ...
  for t in times:
    ...
    if len( Qsgnht ) >= t:
      ...
      errors_mu.append([0.5*np.sum( np.abs(td-at) )])
      ...
      errors_gamma.append([0.5*np.sum( np.abs(td-at) )])
