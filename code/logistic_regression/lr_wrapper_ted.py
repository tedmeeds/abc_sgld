...
class LogisticRegressionProblem(object):
    def __init__(self, lr, K, gamma, q):
        self.lr = lr
        self.N = len(lr.X)
        self.K = K
        self.gamma=gamma
    ...
    def propose(self, theta, params):
        #q = params['propose']['q']
        return theta + self.q*np.random.randn(len(theta)) #np.random.multivariate_normal(theta, q**2*np.eye(len(theta)))

    def loglike_prior(self, theta, params):
        #gamma = params['prior']['gamma']
        return -0.5*self.gamma*theta**2
